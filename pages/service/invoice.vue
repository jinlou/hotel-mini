		...
		methods: {
			...
			submit() {
				if(this.type) {
					...
					if(!this.title1 || !this.phone || !this.taxNumber) {
						uni.showToast({
							icon: 'none',
							title: '请完善信息'
						})
						...
						this.$api.companyInvoice({
							hotelRoomId: uni.getStorageSync('roomInfo').roomId,
							contactNumber: this.phone,
							companyName: this.title1,
							taxNumber: this.taxNumber,
							remark: this.content,
							invoiceAmount: this.price,
							companyAddress: this.companyAddress,
							companyTel: this.telephone,
							bankDeposit: this.bankName,
